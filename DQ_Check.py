...
def readConfig(configXlsFile):
    ...
    global df_dq
    global df_code_cfg
    df_tab = pd.read_excel(configXlsFile,"索引")
    df_dq  = pd.read_excel(configXlsFile,"配置")
    df_code_cfg = pd.read_excel(configXlsFile,"代码")
    #print(df_tab)
    #print(df_dq)
    #return df_tab,df_dq
...
def analyzeFrequency(tabName):
    print("\r\n离散变量频数分析")
    df_freq_col = df_dq[(df_dq["表名"]==tabName) & (df_dq["离散/连续变量"]=="离散")]
    #print(df_freq_col);
...
def main():
    readConfig('C:\\Users\\XingKaiJiang\\Desktop\\pydq\\dq_template.xlsx')
    checkDataQuality()
    #print(df_tab)
    #print(df_dq)
